      ...
      try {
        ...
        if (!userId) {
          try {
            ...
            log(
              "Error looking up user by email for customer " +
        ...
        if (!userId) {
          log(
            "Skipping subscription " +
              subscription.id +
              " - no appwrite_user_id in metadata and could not match by email"
          );
          continue;
        }
        ...
        const docMetadata = Object.assign(
          ...
          { product_label: productLabel || product?.metadata?.label }
        );
