...
module.exports = async ({ req, res, log, error }) => {
  ...
  try {
    ...
    try {
      ...
      const appwriteSubs = await databases.listDocuments(DATABASE_ID, SUBSCRIPTIONS_COLLECTION_ID, [
        sdk.Query.equal("user_id", user.$id),
        sdk.Query.limit(1),
      ]);
        ...
        if (subId) {
          ...
          try {
            ...
            if (
              ...
              log(
                "Stripe subscription found but status is " + (stripeSub ? stripeSub.status : "null")
              );
            }
          } catch (e) {
    ...
    if (!activeSubscription) {
      ...
      activeSubscription = (subscriptionsList.data || []).find(
            ...
            try {
              ...
              await databases.updateDocument(
                ...
                appwriteSubscriptionDoc.$id,
                {
                  ...
                  billing_end_date: updated.current_period_end
                    ? updated.current_period_end.toString()
                    : null,
                  metadata: JSON.stringify(docMetadata),
                }
              );
              log(
                "Synced update to Appwrite subscription document: " + appwriteSubscriptionDoc.$id
              );
            } catch (syncErr) {
